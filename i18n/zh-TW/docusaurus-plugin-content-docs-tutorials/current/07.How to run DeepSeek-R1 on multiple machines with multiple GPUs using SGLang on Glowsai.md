...
頁面底部點選 `Mount`，將 `Datadrive` 掛載到該實例。以利在多台實例間共享檔案，也方便將資料永久保存或下載至本地。

接著點選 `Complete Checkout` 完成租用設定。

![Mount Data Drive](../../../../tutorials-images/07.DeepSeekSGLang/04.MountDataDrive.png)

由於要跨多台實例與多張 GPU 執行 SGLang 推理服務，因此需重複上述步驟，再建立一台實例。當兩台實例都建立完成後，可在 **My Instances** 頁面看到兩筆紀錄。
...
狀態變成 `Connected` 時表示內網連接成功。您會看到每台機器的內部 IP (IP CIDR)，例如下圖中分別為 192.168.1.1 與 192.168.1.2。
...
```bash
# 實例1中依次運行
# 注意：以下指令中的 GLOO_SOCKET_IFNAME、NCCL_SOCKET_IFNAME值請改為您自己輸入 ip -4 a 查出的網絡接口名稱
...
在使用完機器並妥善保存相關環境或資料後，請至 **My Instances** 頁面，點選 `Release` 按鈕以關閉實例。實例關閉後將停止計費，避免資源浪費。
